...
from azure.mgmt.compute.models import (
    VirtualMachine, VirtualMachineSizeTypes, DiskCreateOption, LinuxConfiguration,
...
def create_vm1(subscription_id, RESOURCE_GROUP_NAME, LOCATION):
    #os.environ["AZURE_SUBSCRIPTION_ID"] = "7eed0ac8-1912-4c99-af68-720b126c8599"
    ...
    # RESOURCE_GROUP_NAME = "PythonAzureExample-VM-rg1111123"
    # LOCATION = "westus2"
    ...
    VNET_NAME = "python-example-vnet1"
    SUBNET_NAME = "python-example-subnet1"

    unique_id1 = uuid.uuid4().hex[:8]
    IP_NAME = f"python-example-ip-{unique_id1}"
    IP_CONFIG_NAME = f"python-example-ip-config-{unique_id1}"
    NIC_NAME = f"python-example-nic-{unique_id1}"
    ...
    VM_NAME = f"ExampleVM-{unique_id2}"
    USERNAME = f"azureuser_{unique_id2[:5]}"  # Username with a unique suffix
    PASSWORD = uuid.uuid4().hex[:11] + random.choice("!@#$%^&*")
    ...
    return ip_address_result.ip_address, USERNAME, PASSWORD, VM_NAME
...
def stop_vm(resource_group_name, vm_name):
    ...
    print(f"Stopping VM in Resource group {resource_group_name}")
...
def start_vm(resource_group_name, vm_name):
    ...
    print(f"Starting VM in Resource group {resource_group_name}")
